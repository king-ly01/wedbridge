"""企业微信连接模块 - 移植自原始 bridge 代码"""

import asyncio


def test_wecom_connection(bot_id: str, secret: str) -> tuple:
    """
    ...
    try:
        ...
        async def check():
            try:
                ...
                await client.disconnect()
                return False, "AUTH_FAILED:连接成功但认证失败，请检查 Bot ID 和 Secret 是否正确，或确认机器人是否已在企业微信后台启用"
...
def wait_for_chatid(bot_id: str, secret: str, timeout: int = 60) -> tuple:
    """
    等待用户发送消息，捕获 chatid
        ...
        async def wait():
            try:
                ...
                if not client.is_authenticated:
                    return False, "AUTH_FAILED:企业微信认证失败，请检查 Bot ID 和 Secret 是否正确，或确认机器人是否已在企业微信后台启用"

                for _ in range(timeout * 10):
                    if chatid_received:
                        # 检测到消息，立即返回成功，断开连接在后台执行
...
def send_welcome_message(bot_id: str, secret: str, chatid: str, owner: str, bot_name: str) -> bool:
    """
    发送欢迎消息到企业微信机器人
...
def send_text_message(bot_id: str, secret: str, chatid: str, content: str) -> bool:
    """
    发送文本消息到企业微信机器人（使用 markdown 格式）
    """
    try:
        from wecom_aibot_sdk import WSClient, DefaultLogger
